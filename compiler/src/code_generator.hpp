...
public:
    ...
    void generate_operators(TokenTypes type){
        ...
        if(type == TokenTypes::_addition)
            add_instruction(ASMNode{.instruction=Instructions::_add, .data={Regs::rax, Regs::rbx}});
        if(type == TokenTypes::_subtraction)
            add_instruction(ASMNode{.instruction=Instructions::_sub, .data={Regs::rax, Regs::rbx}});
        if(type == TokenTypes::_multiplication)
            add_instruction(ASMNode{.instruction=Instructions::_imul, .data={Regs::rbx}});
        if(type == TokenTypes::_division)
            add_instruction(ASMNode{.instruction=Instructions::_idiv, .data={Regs::rbx}});
        push(Regs::rax);
    }
    ...
    void generate_sign(std::shared_ptr<ParseNodes::BinSign> bin_sign, std::string label){
        if(bin_sign->sign.type == TokenTypes::_cmp_eq) 
            add_instruction(ASMNode{.instruction=Instructions::_jne, .data={label}});
        if(bin_sign->sign.type == TokenTypes::_cmp_geq) 
            add_instruction(ASMNode{.instruction=Instructions::_jnae, .data={label}});
        if(bin_sign->sign.type == TokenTypes::_cmp_gt)
            add_instruction(ASMNode{.instruction=Instructions::_jng, .data={label}});
        if(bin_sign->sign.type == TokenTypes::_cmp_leq) 
            add_instruction(ASMNode{.instruction=Instructions::_jnbe, .data={label}});
        if(bin_sign->sign.type == TokenTypes::_cmp_lt)
            add_instruction(ASMNode{.instruction=Instructions::_jnl, .data={label}});
        if(bin_sign->sign.type == TokenTypes::_cmp_neq) 
            add_instruction(ASMNode{.instruction=Instructions::_je, .data={label}});
    }
    ...
    void generate_exprbin(const std::shared_ptr<ParseNodes::ExprBin> bin_expression, const std::string& label){
        ...
        add_instruction(ASMNode{.instruction=Instructions::_cmp, .data={Regs::rax, Regs::rbx}});
    ...
    std::string generate_program(){
        add_instruction(ASMNode{.instruction=Instructions::_global, .data={"global _start"}});
        add_instruction(ASMNode{.instruction=Instructions::_label, .data={"_start:"}});
        ...
        add_instruction(ASMNode{.instruction=Instructions::_mov, .data={Regs::rax, "60"}});
        add_instruction(ASMNode{.instruction=Instructions::_mov, .data={Regs::rdi, "0"}});
        add_instruction(ASMNode{.instruction=Instructions::_syscall, .data={""}});

        streamify_code();
        return out.str();
    }

    void push(const Regs reg){
        add_instruction(ASMNode{.instruction=Instructions::_push, .data={reg}});
        stack_loc++;
    }

    void pop(const Regs reg){
        if(stack_loc <= 0){
            std::cerr << "value not ton stack!" << std::endl;
            exit(EXIT_FAILURE);
        }

        add_instruction(ASMNode{.instruction=Instructions::_pop, .data={reg}});
        stack_loc--;
    }
    ...
    void update_variable(const std::string& var){
        ...
        add_instruction(ASMNode{.instruction=Instructions::_mov, .data={Regs::rbx, Regs::rsp}});
        add_instruction(ASMNode{.instruction=Instructions::_mov, .data={Regs::rax, std::to_string(ammount)}});
        add_instruction(ASMNode{.instruction=Instructions::_add, .data={Regs::rsp, Regs::rax}});
        add_instruction(ASMNode{.instruction=Instructions::_mov, .data={Regs::rax, Regs::rcx}});
        add_instruction(ASMNode{.instruction=Instructions::_push, .data={Regs::rax}});
        add_instruction(ASMNode{.instruction=Instructions::_mov, .data={Regs::rsp, Regs::rbx}});
    }

    void add_instruction(ASMNode ins){
    ...
    void streamify_code(){
        ...
        InstructionVisitator iv(out);

        for(ASMNode ins : instructions){
            out << conv_instruction_type(ins.instruction).str();
            int ammount = ins.data.size();

            for(int i = 0; i<ammount; i++){
                std::visit(iv, ins.data[i]);
                if(i + 1 < ammount){
                    out << ", ";
                }
            }
...
private:
    ...
    ExprVisitor(CodeGenerator* _gen) : gen(_gen) {}
        ...
        void operator()(const std::shared_ptr<ParseNodes::ExprIntLit> int_lit){
            std::string data = int_lit->int_lit.data.value();
            gen->add_instruction(ASMNode{.instruction=Instructions::_mov, .data={Regs::rax, data}});
            gen->push(Regs::rax);
        }
        ...
        void operator()(const std::shared_ptr<ParseNodes::ExprIdent> ident_expr){
            ...
            std::string data = "QWORD [rsp+" + std::to_string(offset * 8) + "]";
            gen->add_instruction(ASMNode{.instruction=Instructions::_mov, .data={Regs::rax, data}});
            gen->push(Regs::rax);
        }
    ...
    struct StmtVisitor{
        ...
        void operator()(const std::shared_ptr<ParseNodes::StmtExit> exit_statement){
            ...
            gen->add_instruction(ASMNode{.instruction=Instructions::_mov, .data={Regs::rax, "60"}});
            gen->pop(Regs::rdi);
            gen->add_instruction(ASMNode{.instruction=Instructions::_syscall, .data={""}});
        }
        ...
        void operator()(const std::shared_ptr<ParseNodes::StmtIf> if_statement){
            ...
            gen->add_instruction(ASMNode{.instruction=Instructions::_label, .data={label + ":"}});
        }
        ...
        void operator()(const std::shared_ptr<ParseNodes::StmtGoto> goto_statement){
            ...
            gen->add_instruction(ASMNode{.instruction=Instructions::_jmp, .data={label}});
        }
        ...
        void operator()(const std::shared_ptr<ParseNodes::StmtWhile> while_statement){
            ...
            gen->if_id++;
            gen->add_instruction(ASMNode{.instruction=Instructions::_label, .data={startLabel + ":"}});
            gen->generate_exprbin(while_statement->expression, endLabel);
            gen->generate_statement(while_statement->statement);
            gen->add_instruction(ASMNode{.instruction=Instructions::_jmp, .data={startLabel}});
            gen->add_instruction(ASMNode{.instruction=Instructions::_label, .data={endLabel + ":"}});
        }
        private:
        CodeGenerator* gen;
    };

    struct InstructionVisitator{
        InstructionVisitator(std::stringstream& _data): data(_data) {}

        void operator()(const Regs reg){
            data << register_name[reg];
        }
